...
<script type="text/javascript">
	...
	$(function(){
			...
			$.ajax({
				...
				success:function (data){
					...
					if (data.code=="SUCCESS"){
						var html= "";
						 html+="<tr>";
						 html+="<td><input type=\"checkbox\" id=\""+data.retDate.id+"\"/></td>"
						 html+="<td><a style=\"text-decoration: none; cursor: pointer;\" onclick=\"window.location.href='workbench/clue/detailClue.do?id="+data.retDate.id+"'\">"+data.retDate.fullname+""+data.retDate.appellation+"</a></td>";
						 html+="<td>"+data.retDate.company+"</td>";
						 html+="<td>"+data.retDate.phone+"</td>";
						 html+="<td>"+data.retDate.mphone+"</td>";
						 html+="<td>"+data.retDate.source+"</td>";
						 html+="<td>"+data.retDate.owner+"</td>";
						 html+="<td>"+data.retDate.state+"</td>";
						 html+="</tr>";
						 $("#abc").html(html);
		...
		$("#abc").on("click","input[type='checkbox']",function () {
			alert("傻逼");
		})
	});
	...
	function chushihua(){
		$.ajax({
			...
			success:function (data) {
				var html= "";
				$.each(data.retDate,function (indedx,obj) {
					html += "<tr>";
				...
				//这个下面的函数代码，我没有做的很详细，只是为了我进行后面的业务，这段代码的好多地方我都写死了，没有动态获取
				//其实后面的这些参数都应该在ajax返回回来的data数据中去获取
				$("#ssbb").bs_pagination({
					currentPage:1,               //当前页的页号   相当于pageNo
					rowsPerPage:10,             //每页显示的记录数   相当于pageSize
					totalRows:100,              //总记录条数
					totalPages:20,              //总页数，必填参数
					...
					onChangePage:function (event,pageobj){    //用户每次切换页号的时候，都会自动的执行这个函数
						...
						chushihua(pageobj.currentPage,pageobj.rowsPerPage);
					}
				})
			}
		})
...
<body>
					...
					<form id="createClueForm" class="form-horizontal" role="form">
							...
							<div class="col-sm-10" style="width: 300px;">
								<select class="form-control" id="create-appellation">
								 ...
								 <c:forEach items="${appellationlist}" var="a">
									 <option name="abc" value="${a.id}">${a.value}</option>
								 </c:forEach>
								</select>
							</div>
						...
						<div style="position: relative;top: 15px;">
							...
							<div class="form-group">
								...
								<div class="col-sm-10" style="width: 300px;">
									<input type="text" class="form-control  dayDate" id="create-nextContactTime"   readonly>
								</div>
							</div>
						</div>
	...
	<div style="position: relative; top: -20px; left: 0px; width: 100%; height: 100%;">
		...
		<div style="width: 100%; position: absolute;top: 5px; left: 10px;">
			...
			<div id="ssbb" style="height: 50px; position: relative;top: 60px;">
				<%--<div>
